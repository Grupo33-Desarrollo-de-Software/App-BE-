...
class CalificacionesTests(TestCase):
    ...
    def test_rate_album(self, mock_parsear_artista, mock_get_artista, mock_buscar_album, mock_parsear_fecha, mock_log):
        ...
        mock_log.assert_called_once_with("El usuario testuser calificó el album Test Album")
    ...
    @patch("albums.views.api.getArtista")
    @patch("albums.views.api.parsearArtista2")
    def test_modify_rating(self, mock_parsear_artista, mock_get_artista, mock_buscar_album, mock_parsear_fecha, mock_log):
        Rate.objects.create(
            usuario=self.user,
            album=self.album,
            rate=3,
...
class IntegrationTests(TestCase):
    ...
    def test_rate_lifecycle(
        self, mock_buscar_album, mock_persistir, mock_parsear_fecha, mock_log_action
    ):
        ...
        mock_log_action.assert_called_once_with(f"El usuario testuser calificó el album {album_title}")
        ...
        self.assertEqual(Rate.objects.filter(usuario=self.user, album=album_obj).count(), 1)
        ...
        mock_log_action.assert_called_once_with("El usuario testuser calificó el album Integration Album")
