...
class FollowListTests(TestCase):
    ...
    def test_add_album_to_follow_list(
        ...
        artista_nuevo = Artista.objects.create(
            name="New Artist", image="http://example.com/new_artist.jpg"
        )
        ...
        follow_exists = Follow.objects.filter(
            usuario=self.user, album=album_nuevo
        ).exists()
        self.assertTrue(follow_exists)

        # nos fijamos q se haya logueado y notificado bien
        mock_log.assert_called_once_with(
            "el usuario testuser siguió el album New Album"
        )
        mock_notif.assert_called_once_with(
            self.user,
            "Seguido con éxito",
            f"Has seguido con éxito el album New Album de New Artist",
        )
    ...
    def test_remove_album_from_follow_list(
        ...
        follow_exists = Follow.objects.filter(
            usuario=self.user, album=self.album1
        ).exists()
        self.assertFalse(follow_exists)
        mock_log_action.assert_called_once_with(
            "el usuario testuser dejó de seguir el album Test Album 1"
        )
...
class IntegrationTests(TestCase):
    ...
    def test_cycle_follow_and_unfollow(
        ...
        response_follow = self.client.get(
            "/api/v1/follow/Integration%20Artist/Integration%20Album"
        )
        self.assertEqual(response_follow.status_code, status.HTTP_200_OK)

        # verificamos q el Follow existe en la db
        self.assertTrue(
            Follow.objects.filter(usuario=self.user, album=album_obj).exists()
        )

        # verificamos los logs y las notifs de esto
        mock_log_action.assert_called_once_with(
            f"el usuario testuser siguió el album {album_title}"
        )
        mock_notif.assert_called_once_with(
            self.user,
            "Seguido con éxito",
            f"Has seguido con éxito el album {album_title} de {artist_name}",
        )
        ...
        response_unfollow = self.client.get(
            "/api/v1/unfollow/Integration%20Artist/Integration%20Album"
        )
        ...
        mock_log_action.assert_called_once_with(
            f"el usuario testuser dejó de seguir el album {album_title}"
        )
